...
# ClientGui needs its own twisted reactor, so if we need it,
# then install the gui reactor before importing the other
# modules that use twisted
#
if ('-g' in sys.argv) or ('--gui' in sys.argv):
    import cb.gui.client.clientmain
    print 'Starting the GUI'
    GUI = cb.gui.client.clientmain.init()
else:
    GUI = None
...
# necessary to run client.py as the same user each time, and some
...
def run_client_agent(decoy_host, decoy_port, tunnel_port,
    ...
    if os.path.exists('nss-bin/client-agent.exe'):
        cmd = "cd nss-bin && client-agent.exe -h %s -p %d %s" % (
                decoy_host, decoy_port, deadbeef)
        if not deadbeef:
            cmd += ' -s %s' % sentinel
        use_shell = True
    else:
    ...
    def check_client_agent():
        ...
        if exit_code != None:
            ...
            try:
                ...
                print "client.py.check_client_agent: reactor already stopped"
...
def start_cbsmd(opts):
    ...
    NOTE: client.py kiils all of its descendants in atexit,
...
def main(gui):
    ...
    # Note that we need to delay these imports until after we start
    # the gui reactor (if we start the gui at all) in order to get
    # the twisted reactor initialized properly.
    #
	...
	if PLATFORM == 'win32':
	    ...
	    cmd = 'su -c "/system/xbin/sh %s %s"' % (
                    os.path.join(DIRNAME, 'pin-tcp4-route'),
		    socket.gethostbyname(decoy_addr[0]))
	    os.system(cmd)
	else:
    ...
    if opts.gui:
        ccp_client.signals.register('CT_CONNECTED', gui.tunnel_up)
        ccp_client.signals.register('TPUT_UPDATE', gui.tput_update)
    if isAndroid:
